...
class Show {
    ...
    */
     ...
     */
    public boolean compareMerge( Show a, Show b ) {
        boolean result = comparelessthan(a.get_duration(),b.get_duration());
        if ( a.get_duration().compareTo(b.get_duration()) == 0 ) {
     ...
     * Ordena disneyplus
     */
    private void mergesort(int esq, int dir, ArrayList<Show> disneyplus, Log log) {
        if (esq < dir)  {
            int meio = (esq + dir) / 2;
            mergesort(esq, meio, disneyplus, log);
            mergesort(meio + 1, dir, disneyplus, log);
            intercalar(esq, meio, dir, disneyplus, log);
        }
    }
    ...
    */ 
    public void intercalar(int esq, int meio, int dir, ArrayList<Show> disneyplus, Log log){
        int n1, n2, i, j, k;
        ...
        ArrayList<Show> a1 = new ArrayList<Show>();
        ArrayList<Show> a2 = new ArrayList<Show>();
        createDefaultArrayList(a1, n1+1);
        createDefaultArrayList(a2, n2+1);

        //Inicializar primeiro subarray
        for(i = 0; i < n1; i++){
            a1.set(i,disneyplus.get(esq+i));
            log.setMovimentacoes(log.getMovimentacoes()+1);
        }

        //Inicializar segundo subarray
        for(j = 0; j < n2; j++){
            a2.set(j,disneyplus.get(meio+j+1));
            log.setMovimentacoes(log.getMovimentacoes()+1);
        }
        ...
        for(i = j = 0, k = esq; k <= dir; k++){
            if ( i >= n1 ) {
                ...
                tmp = compareMerge(a1.get(i),a2.get(j)) ? a1.get(i++) : a2.get(j++);
            }
            log.setComparacoes(log.getComparacoes()+1);
            disneyplus.set(k,tmp);
            log.setMovimentacoes(log.getMovimentacoes()+1);
        }
    }
    ...
    public static void main ( String args[] ) {
        try {
            ...
            double now, time;
            FileWriter logPtr = new FileWriter("matricula_mergesort.txt");
            Log log = new Log();            
            ...
            useless.mergesort(0, disneyplus.size()-1, disneyplus, log);
            for ( int y = 0; y < disneyplus.size(); y++ ) {
                //System.out.println(disneyplus.get(y).get_duration() + " " + disneyplus.get(y).get_title() );
