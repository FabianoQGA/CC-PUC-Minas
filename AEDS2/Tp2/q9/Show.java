...
class Show {
    ...
    */
     ...
     */
    public boolean compareHeap( Show a, Show b ) {
        boolean result;
        if (a != null && b == null) {
            result = true;
        }
        ...
        result = comparelessthanarray(b.get_director(),a.get_director());
        if ( compareequalarray(b.get_director(), a.get_director()) ) {
            result = comparelessthan(b.get_title(),a.get_title());  
        }
        return result;
    }
     ...
     */
    public void heapSort( ArrayList<Show> disneyplus, Log log ) {
        int n = disneyplus.size();
        Show tmp = new Show();
        disneyplus.add(tmp);
        for ( int i = disneyplus.size()-1; i > 0; i-- ) {
            disneyplus.set(i,disneyplus.get(i-1));
        } // pos 0 fica inutilizada

        //Contrucao do heap
        for(int tamHeap = 2; tamHeap <= n; tamHeap++){
            construir(tamHeap,disneyplus);
        }

        /*
        for ( int y = 1; y < disneyplus.size(); y++ ) {
            System.out.print(disneyplus.get(y).get_id() + " indice[" + y + "]" + " F1, " + y*2 + " F2, "+ (y*2+1) + " director=> " + disneyplus.get(y).get_director()[0] + " title=> " + disneyplus.get(y).get_title());
            System.err.println("");
        } */
        ...
        int tamHeap = n;
        while(tamHeap > 1){
            Show temp = disneyplus.get(1);
            disneyplus.set(1, disneyplus.get(tamHeap));
            disneyplus.set(tamHeap, temp);
            tamHeap--;
            reconstruir(tamHeap, disneyplus);
        }

        for ( int i = 0; i < n; i++ ) {
            disneyplus.set(i,disneyplus.get(i+1));
        }
        disneyplus.remove(disneyplus.size()-1);
    }

    /**
     * Constroi o heap
     */
    public void construir(int tamHeap, ArrayList<Show> disneyplus ){ // array[i] > array[i/2]
        for(int i = tamHeap; i > 1 && compareHeap(disneyplus.get(i), disneyplus.get(i/2)); i /= 2){
            Show temp = disneyplus.get(i/2);
            disneyplus.set(i/2, disneyplus.get(i));
            disneyplus.set(i, temp);
        }
    }
     ...
     * Reconstroi heap 
     */
    public void reconstruir(int tamHeap, ArrayList<Show> disneyplus){
        int i = 1;
        while(i <= (tamHeap/2)){
            int filho = getMaiorFilho(i, tamHeap, disneyplus);
            if(compareHeap(disneyplus.get(filho), disneyplus.get(i))){ //array[i] < array[filho]
                Show temp = disneyplus.get(i);
                disneyplus.set(i, disneyplus.get(filho));
                disneyplus.set(filho, temp);
                i = filho;
            }else{
                i = tamHeap;
            }
    ...
    /**
     * Retorna indice do maior filho de um heap
     */
    public int getMaiorFilho(int i, int tamHeap, ArrayList<Show> disneyplus){
        int filho;
        if (2*i == tamHeap || compareHeap(disneyplus.get(2*i), disneyplus.get(2*i+1)) ){ //array[2*i] > array[2*i+1]
            filho = 2*i;
        } else {
            filho = 2*i + 1;
        }
    ...
    public static void main ( String args[] ) {
        try {
            ...
            double now, time;
            FileWriter logPtr = new FileWriter("matricula_heapsort.txt");
            Log log = new Log();            
            ...
            useless.heapSort(disneyplus,log);
