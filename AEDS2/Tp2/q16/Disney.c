 ...
 */
bool compareInsertion ( Show *a, Show *b ) {
    bool result;
    result = comparelessthan(a->type,b->type);
 ...
 * Ordena array de strings alfabeticamente
 */
void insertionsort (Show *disneyplus[], int size) {
    for (int i = 1; i < size; i++) {
        Show *tmp = disneyplus[i];
        int j = i < 10 ? (i - 1) : 9;
  
        while ( (j >= 0) && compareInsertion(tmp,disneyplus[j]) ) {
            disneyplus[j+1] = createClone(disneyplus[j]);
            j--;
            comparacoes++;
            movimentacoes++;
        }
...
int main ( ) {
    ...
    FILE *ptr = fopen("matricula_insercao.txt", "w");
    if ( ptr == NULL) {
