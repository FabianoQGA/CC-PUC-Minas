 ...
 */
void quicksortRec(Show *disneyplus[], int esq, int dir) {
    int i = esq, j = dir;
    Show *pivo = disneyplus[(dir+esq)/2];
    while (i <= j) {
        while (compareQuick(disneyplus[i], pivo)) {
            i++;
            comparacoes++;
        }
        while (compareQuick(pivo,disneyplus[j])) {
            j--;
            comparacoes++;
        } 
        if (i <= j) {
            Show *tmp = disneyplus[i];
            disneyplus[i] = disneyplus[j];
            disneyplus[j] = tmp;
            movimentacoes += 3;
            i++;
            j--;
    ...
    if (esq < j)  quicksortRec(disneyplus, esq, j);
    if (i < dir)  quicksortRec(disneyplus, i, dir);
}

/**
 * Ordena disneyplus
 */
void quicksort(Show *disneyplus[], int size) {
    quicksortRec(disneyplus, 0, size-1);
}
...
int main ( ) {
    ...
    FILE *ptr = fopen("matricula_quicksort.txt", "w");
    if ( ptr == NULL) {
