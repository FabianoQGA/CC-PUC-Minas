...
/**
 * Compara directors e desempata com title
 */
bool compareHeap(Show *a, Show *b) {
    if ( !a && b ) {
        return true;
    } else if ( a && !b ) {
        return false;
    }
    bool result = comparearray(a->director,b->director) == 1 ? true : false;
    comparacoes += 2;
    if ( comparearray(a->director,b->director) == 0 ) {
 ...
 * Constroi heap
 */
void construir(Show *disneyplus[], int tamHeap){
    for(int i = tamHeap; i > 1 && compareHeap(disneyplus[i/2],disneyplus[i]); i /= 2){
        Show *tmp = disneyplus[i];
        disneyplus[i] = disneyplus[i/2];
        disneyplus[i/2] = tmp;
        movimentacoes += 3;
    }
}
 ...
 * Retorna maior filho
 */
int getMaiorFilho(Show *disneyplus[], int i, int tamHeap){
    int filho;
    if (2*i == tamHeap || compareHeap(disneyplus[2*i+1],disneyplus[2*i])){
        filho = 2*i;
    } else {
        filho = 2*i + 1;
    }
 ...
 * Reconstroe o heap
 */
void reconstruir(Show *disneyplus[], int tamHeap){
    int i = 1;
    while(i <= (tamHeap/2)){
        int filho = getMaiorFilho(disneyplus, i, tamHeap);
        if(compareHeap(disneyplus[i],disneyplus[filho])){
            Show *tmp = disneyplus[i];
            disneyplus[i] = disneyplus[filho];
            disneyplus[filho] = tmp;
            i = filho;
            movimentacoes += 3;
        }else{
            i = tamHeap;
 ...
 * Ordena disneyplus
 */
void heapsort(Show *disneyplus[], int size) {

    //Alterar o vetor ignorando a posicao zero
    Show *arrayTmp[401] = { NULL }; // check initializion
    if (arrayTmp == NULL) {
        perror("arrayTmp MEMORY ALLOCATION ERROR");
        free(arrayTmp);
    }
    for(int i = 0; i < size; i++){
        arrayTmp[i+1] = disneyplus[i];
    }
    ...
    for ( int i = 11; i <= size; i++ ) {
        if ( compareHeap(disneyplus[i],disneyplus[1]) ) {
            Show *tmp = disneyplus[1];
            arrayTmp[1] = arrayTmp[i];
            arrayTmp[i] = tmp;
            movimentacoes += 3;
            reconstruir(disneyplus,10);
        }
    }
...
int main ( ) {
    ...
    FILE *ptr = fopen("matricula_heapsort.txt", "w");
    if ( ptr == NULL) {
