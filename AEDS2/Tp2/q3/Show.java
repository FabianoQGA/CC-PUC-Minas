...
class Show {
    ...
    */
    ...
    public boolean searchTitle ( ArrayList<Show> disneyplus, String s, Log log ) {
        boolean result = false;
        for ( int y = 0; y < disneyplus.size() && ! result; y++ ) {
            if ( disneyplus.get(y).get_title().compareTo(s) == 0 ) {
                result = true;
            }
    ...
    public static void main ( String args[] ) {
        try {
            ...
            double now, time;
            FileWriter logPtr = new FileWriter("matrícula\\_sequencial.tx");
            Log log = new Log();
            ...
            s = o.nextLine();
            if (useless.searchTitle(disneyplus,s,log)) {
