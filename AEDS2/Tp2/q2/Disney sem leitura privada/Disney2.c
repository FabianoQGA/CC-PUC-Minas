 ...
 */
void nameselection (char** s, int n) {
    char* temp;
    for ( int i = 0; i < n-1; i++ ) {
        int menor = i; 
        for ( int j = i+1; j < n; j++ ) {
 ...
 * faz sptr apontar para o novo objeto de show
 */
void read (char *s, Show *sptr) {
    FILE *dptr;
...
int main ( ) {
    ...
    Show *sptr = malloc(1400*sizeof(Show));
    if (sptr == NULL) {
        perror("SPTR MEMORY ALLOCATION ERROR");
        fclose(ptr);
        free(sptr);
        return 1;
    }
