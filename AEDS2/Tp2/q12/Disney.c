 ...
 */
bool comparebubble(Show *a, Show *b) {
 ...
 * Ordena disneyplus
 */
void bubblesort(Show *disneyplus[], int size) {
    int i, j;
    for (i = 0; i < (size-1); i++) {
        for (j = 0; j < (size-1); j++) {
            comparacoes++;
            if ( comparebubble(disneyplus[j+1], disneyplus[j]) ) {
                //Show *temp = createClone(disneyplus[j]);
                //disneyplus[j] = createClone(disneyplus[j+1]);
                //disneyplus[j+1] = temp;
                Show *temp = disneyplus[j];
                disneyplus[j] = disneyplus[j+1];
                disneyplus[j+1] = temp;
                movimentacoes += 3;
            }
        }
...
int main ( ) {
    ...
    FILE *ptr = fopen("matricula_bolha.txt", "w");
    if ( ptr == NULL) {
    ...
    bubblesort(disneyplus,n);
    //printf("%d %d %d\n", disneyplus[5]->date_added.year, disneyplus[5]->date_added.month, disneyplus[5]->date_added.day);
    for ( int y = 0; y < n; y++ ) {
        print(disneyplus[y]);
    }
