 ...
 * Ordena array de strings alfabeticamente
 */
void disneyplusselection (Show *disneyplus, int size) {
    for ( int i = 0; i < size-1; i++ ) {
        int menor = i; 
        for ( int j = i+1; j < size; j++ ) {
...
void print( Show *show ) {
    printf("=> %s ## %s ## %s ## [", show->id, show->type, show->title);
...
/**
 * Pesquisa por um nome em disneyplus
 */
bool pesquisa (Show *disneyplus, char *s ) {
    bool resp = false;
    int dir = n - 1, esq = 0, meio;
    while (esq <= dir) {
        meio = (esq + dir) / 2;
        if (strcmp(s,(disneyplus+meio)->title) == 0 ){
            resp = true;
            esq = n;
            comparacoes++;
        } else if (strcmp(s,(disneyplus+meio)->title) > 0){
            esq = meio + 1;
            comparacoes += 2;
        } else {
            dir = meio - 1;
...
int main ( ) {
    ...
    FILE *ptr = fopen("matrícula_binaria.txt", "w");
    if ( ptr == NULL) {
    ...
    disneyplusselection(disneyplus,n);

    fgets(s, sizeof(s), stdin);
    s[strcspn(s, "\n")] = '\0';

    if ( pesquisa(disneyplus,s) ) {
        printf("SIM\n");
    } else {
        printf("NAO\n");
    }
