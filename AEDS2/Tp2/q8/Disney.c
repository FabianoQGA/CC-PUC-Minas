...
/**
 * Compara types e desempata com title
 */
bool compareshell(Show *a, Show *b) {
    bool result = comparelessthan(a->type,b->type);
 ...
 * Ordena disneyplus
 */
void insercaoPorCor(Show *disneyplus[], int size, int cor, int h){
    for (int i = (h + cor); i < size; i+=h) {
        Show *tmp = disneyplus[i];
        int j = i - h;
        while ( (j >= 0) && (compareshell(tmp,disneyplus[j])) ) {
            disneyplus[j + h] = createClone(disneyplus[j]);
            j-=h;
        }
        disneyplus[j + h] = createClone(tmp);
        freeShow(tmp);
    }
}

void shellsort(Show *disneyplus[], int size) {
    int h = 1;
    ...
    do {
        ...
        for(int cor = 0; cor < h; cor++){
            insercaoPorCor(disneyplus, size, cor, h);
        }
    } while (h != 1);
}
...
int main ( ) {
    ...
    FILE *ptr = fopen("matricula_shellsort.txt", "w");
    if ( ptr == NULL) {
