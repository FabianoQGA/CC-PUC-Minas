...
class Show {
    ...
    */
     ...
     */
    public boolean compareInsertion( Show a, Show b ) {
        boolean result = comparelessthan(b.get_type(),a.get_type());
        if ( a.get_type().compareTo(b.get_type()) == 0 ) {
     ...
     */
    public void insertionSort( ArrayList<Show> disneyplus, Log log ) {
        for (int i = 1; i < disneyplus.size(); i++) {
            Show tmp = disneyplus.get(i);
            int j = i - 1;

            while ((j >= 0) && (compareInsertion(disneyplus.get(j),tmp))) {
                disneyplus.set(j + 1, disneyplus.get(j));
                log.setMovimentacoes(log.getMovimentacoes()+1);
                log.setComparacoes(log.getComparacoes()+1);
                j--;
            }
            disneyplus.set(j + 1, tmp);
            log.setMovimentacoes(log.getMovimentacoes()+1);
        }
    }
    ...
    public static void main ( String args[] ) {
        try {
            ...
            double now, time;
            FileWriter logPtr = new FileWriter("matricula_insercao.txt");
            Log log = new Log();            
            ...
            useless.insertionSort(disneyplus,log);
