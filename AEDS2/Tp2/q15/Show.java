...
class Show {
    ...
    */
     ...
     */
    public void selectionSort( ArrayList<Show> disneyplus, Log log ) {
        for ( int i = 0; i < 10; i++ ) {
            int menor = i;
            for ( int j = i+1; j < disneyplus.size(); j++ ) {
                if ( comparelessthan(disneyplus.get(j).get_title(),disneyplus.get(menor).get_title()) ) {
                    menor = j;
                }
    ...
    public static void main ( String args[] ) {
        try {
            ...
            double now, time;
            FileWriter logPtr = new FileWriter("matricula_selecao.txt");
            Log log = new Log();            
            ...
            useless.selectionSort(disneyplus,log);
            for ( int y = 0; y < 10; y++ ) {
