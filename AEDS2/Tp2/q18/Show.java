...
class Show {
    ...
    */
     ...
     * Ordena disneyplus
     */
    private void quicksort(int esq, int dir, ArrayList<Show> disneyplus, Log log) {
        int i = esq, j = dir;
        Show pivo = disneyplus.get((dir+esq)/2);
        while (i <= j) {
            while ( compareQuick(disneyplus.get(i), pivo) ) {
                log.setComparacoes(log.getComparacoes()+1);
                i++; 
            }
            while ( compareQuick(pivo, disneyplus.get(j)) ) {
                log.setComparacoes(log.getComparacoes()+1);
                j--;
            }
            if (i <= j) {
                Show tmp = disneyplus.get(i);
                disneyplus.set(i,disneyplus.get(j));
                disneyplus.set(j, tmp);
                log.setMovimentacoes(log.getMovimentacoes()+1);
                i++;
                j--;
            }
        }
        if (esq < j)  quicksort(esq, j, disneyplus, log);
        if (i < 10 && i < dir)  quicksort(i, dir, disneyplus, log);
    }
    ...
    public static void main ( String args[] ) {
        try {
            ...
            double now, time;
            FileWriter logPtr = new FileWriter("matricula_quicksort.txt");
            Log log = new Log();            
            ...
            useless.quicksort(0, disneyplus.size()-1, disneyplus, log);
            for ( int y = 0; y < 10; y++ ) {
