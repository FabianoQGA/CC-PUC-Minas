...
/**
 * Ordena disneyplus iguais por nome
 */
void nameSort (Show *disneyplus[], int size) {
    for ( int i = 0; i < size-1; i++ ) {
        int menor = i; 
        for ( int j = i+1; j < size; j++ ) {
            if ( disneyplus[j]->release_year == disneyplus[menor]->release_year && comparelessthan(disneyplus[j]->title, disneyplus[menor]->title) ) {
                menor = j;
            }
        }
        Show *temp = createClone(disneyplus[i]);
        disneyplus[i] = createClone(disneyplus[menor]);
        disneyplus[menor] = temp;
    }
}

/**
 * Mostra qual show é maior
 */
bool compareRadix ( Show *a, Show *b ) {
    bool result;
    result = a->release_year < b->release_year;
...
/**
 * Pega maior release year
 */
int getMax(Show *disneyplus[], int size) {
    int maior = disneyplus[0]->release_year;

    for (int i = 1; i < size; i++) {
        if(maior < disneyplus[i]->release_year){
            maior = disneyplus[i]->release_year;
        }
    }
    return maior;
}
...
/**
 * Counting sort dentro de radixsort
 */
void radcountingSort(Show *disneyplus[], int size, int exp) {
    int count[10];
    Show *output[size];
    ...
    for (int i = 0; i < size; i++) {
        count[( (disneyplus[i]->release_year) /exp) % 10]++;
        movimentacoes++;
    }
    ...
    for (int i = size-1; i >= 0; i--) {
        output[count[( (disneyplus[i]->release_year) / exp) % 10] - 1] = disneyplus[i];
        count[( (disneyplus[i]->release_year) /exp) % 10]--;
        movimentacoes += 2;
    }
    ...
    for (int i = 0; i < size; i++) {
        disneyplus[i] = createClone(output[i]);
    }
}
 ...
 * Ordena disneyplus
 */
void radixsort (Show *disneyplus[], int size) {
    int max = getMax(disneyplus, n);
    for (int exp = 1; max/exp > 0; exp *= 10) {
        radcountingSort(disneyplus, size, exp);
    }
}
...
int main ( ) {
    ...
    FILE *ptr = fopen("matricula_radixsort.txt", "w");
    if ( ptr == NULL) {
    ...
    radixsort(disneyplus,n);
    nameSort(disneyplus,n);
    for ( int y = 0; y < n; y++ ) {
        print(disneyplus[y]);
    }
    ...
    clock_t now = clock();
    time = now - then;
    fprintf(ptr,"782195\t%f\t%d\t%d\n", time, movimentacoes, comparacoes);
    fprintf(ptr,"O Radix Sort é um algoritmo de ordenação não-comparativo que processa dígitos individuais dos números para ordenação. Ele utiliza de um outro algoritmo (countingsort) para ordernar os elementos. \nPasso a passo: *identificar o número máximo *analisar cada dígito do menos significativo ao mais significativo, depois seguir para os outros dígitos (de unidade até dezena etc.)");
