...
void setNivel ( No *no ) {
    int max;
    max = (no->esq == NULL ? 0 : getNivel(no->esq)) > (no->dir == NULL ? 0 : getNivel(no->dir)) ? getNivel(no->esq) : getNivel(no->dir); 
    no->nivel = 1 + max;
}

void deleteRec ( No *no ) {
    if ( no != NULL ) {
        deleteRec(no->esq);
        deleteRec(no->dir);
        freeShow(no->elemento);
        free(no);
    }
}
...
No *inserirRec ( Show *show, No *i ) {
    if ( i == NULL ) {
        i = createNo(show);
    } else if ( comparelessthan(show->title,i->elemento->title) ) {
        i->esq = inserirRec(show, i->esq);
    } else if ( comparelessthan(i->elemento->title,show->title) ) {
        i->dir = inserirRec(show, i->dir);
    }
    return ( balancear(i) );
}

void inserir ( Show *show ) {
    raiz = inserirRec( show, raiz );
}

bool pesquisarRec ( char *title, No *i ) {
    bool result = false;
    if ( i == NULL ) {
        //
        ...
        result = true;
        comparacoes++;
    } else if ( comparelessthan(title,i->elemento->title) ) {
        printf(" esq");
        result = pesquisarRec ( title, i->esq );
        comparacoes += 2;
    } else if ( comparelessthan(i->elemento->title,title) ) {
        printf(" dir");
        result = pesquisarRec ( title, i->dir );
        comparacoes += 3;
    } else {
        comparacoes += 3;
    }
...
bool pesquisar ( char *title ) {
    printf("raiz");
    return pesquisarRec ( title, raiz );
}
...
int main ( ) {
    ...
    FILE *ptr = fopen("matrícula_avl.txt", "w");
    if ( ptr == NULL) {
    ...
    clock_t now = clock();
    time = now - then;
    fprintf(ptr,"782195\t%f\t%d", time, comparacoes);
