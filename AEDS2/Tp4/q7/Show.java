...
class Hash {
    ...
    public Hash(int m) {
        ...
        for (int i = 0; i < m; i++) {
            tabela.add(new Celula());
        }
    }

    public int ascii (String input) {
        int sum = 0;
        for ( int y = 0; y < input.length(); y++ ) {
            sum += (int)input.charAt(y);
        }
        return sum;
    }

    public int h(String elemento) {
        return ascii(elemento) % m;
    }

    public void inserir(Show elemento) {
        if (elemento != null) {
            int pos = h(elemento.get_title());
            if (tabela.get(pos).elemento.get_title().compareTo("") == 0) {
                tabela.get(pos).elemento = elemento;
            } else {
                Celula i = tabela.get(pos).prox;
                boolean inserido = false;
                for ( i = tabela.get(pos).prox; i != null; i = i.prox ) {
                    if ( i.elemento == elemento ) {
                        inserido = true;
                    }
                }
                if ( ! inserido ) {
                    i = new Celula(elemento);
                }
            }
        }
    }

    public boolean pesquisar(String titulo, Log log) {
        boolean resp = false;
        int pos = h(titulo);
        if ( tabela.get(pos) != null && tabela.get(pos).elemento.get_title().compareTo(titulo) == 0 ) {
            resp = true;      
            System.out.println(" (Posicao: " + pos + ") SIM");
            log.setComparacoes(log.getComparacoes()+1);
        } else if ( tabela.get(pos).elemento.get_title().compareTo("") == 0 ) {
            System.out.println(" (Posicao: " + pos + ") NAO");
            log.setComparacoes(log.getComparacoes()+2);
        } else {
            Celula i;
            log.setComparacoes(log.getComparacoes()+2);
            for ( i = tabela.get(pos).prox; i != null && i.elemento.get_title().compareTo(titulo) != 0; i = i.prox ) {
                log.setComparacoes(log.getComparacoes()+1);
            }
            if ( i != null && i.elemento.get_title().compareTo(titulo) == 0 ) {
                resp = true;
            }
            if ( resp ) {
                System.out.println(" (Posicao: " + pos + ") SIM");
...
class Show {
    ...
    */
    ...
    // Ler
    public void ler ( Hash disneyplus, String id_string ) { 
        try {
    ...
    public static void main ( String args[] ) {
        try {
            ...
            double now, time;
            FileWriter logPtr = new FileWriter("matrícula_hashIndireta.txt");
            Log log = new Log();
