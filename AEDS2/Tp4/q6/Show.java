...
class Hash {
    ...
    public int h(String elemento) {
        return ascii(elemento) % m1;
    }

    public int rh(String elemento) {
        return (ascii(elemento)+1) % m1;
    }

    public void inserir(Show elemento) {
        if (elemento != null) {
            int pos = h(elemento.get_title());
            if (tabela.get(pos).get_title().compareTo("") == 0) {
                tabela.set(pos, elemento);
            } else {
                int posR = rh(elemento.get_title());
                if ( tabela.get(posR).get_title().compareTo("") == 0 ) {
                    tabela.set(posR, elemento);
                }
            }
        }
    }
    ...
    public boolean pesquisar(String elemento, Log log) {
        boolean resp = false;
        int pos = h(elemento);
        int posR = rh(elemento);
        if (tabela.get(pos).get_title().compareTo(elemento) == 0 ) {
            resp = true;      
            System.out.println(" (Posicao: " + pos + ") SIM");
            log.setComparacoes(log.getComparacoes()+1);
        } else if (tabela.get(posR).get_title().compareTo(elemento) == 0) {
    ...
    */
    ...
    public static void main ( String args[] ) {
        try {
            ...
            double now, time;
            FileWriter logPtr = new FileWriter("matrícula_hashRehash.txt");
            Log log = new Log();
