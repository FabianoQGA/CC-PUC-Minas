...
class Arvore {
    ...
    public No inserir ( int number, No i ) {
        if ( i == null ) {
            i = new No(number%15);
        } else if ( i.numb < number%15 ) {
    ...
    public No inserir ( Show show, No i ) {
        if ( i == null ) {
            System.out.println("MOD DE 15 INEXISTENTE PARA POKEMON ID " + show.get_id() + " DE RELEASE YEAR " + show.get_release_year() );
        } else if ( i.numb < show.get_release_year()%15 ) {
            i.dir = inserir(show, i.dir);
        } else if ( i.numb > show.get_release_year()%15 ) {
            i.esq = inserir(show, i.esq);
        } else {
            i.raiz2 = inserir( show.get_title(), i.raiz2 );
        }
        return i;
    }

    public No2 inserir ( String title, No2 i ) {
        if ( i == null ) {
            i = new No2(title);
        } else if ( title.compareTo(i.title) > 0 ) {
            i.dir = inserir(title, i.dir);
        } else if ( title.compareTo(i.title) < 0 ) {
            i.esq = inserir(title, i.esq);
        } else {
            //
        }
        return i;
    ...
    public boolean pesquisar ( String title, Log log ) {
        boolean result = false;
        System.out.print("raiz ");
        result = pesquisar( title, raiz, log );
        if ( result ) {
    ...
    public boolean pesquisar ( String title, No i, Log log ) {
        boolean result = false;
        if ( i != null ) {
            result = pesquisar(title, i.raiz2, log);
            System.out.print(" ESQ ");
            result = result || pesquisar(title, i.esq, log);
            System.out.print(" DIR ");
            result = result || pesquisar(title, i.dir, log);
        }
        return result;
    }

    public boolean pesquisar ( String title, No2 i, Log log) {
        boolean result = false;
        if ( i != null ) {
            if ( title.compareTo(i.title) == 0 ) {
                result = true;
                log.setComparacoes(log.getComparacoes()+1);
            } else if ( title.compareTo(i.title) > 0 ) {
                System.out.print("dir ");
                result = pesquisar(title, i.dir, log);
                log.setComparacoes(log.getComparacoes()+2);
            } else if ( title.compareTo(i.title) < 0 ) {
                System.out.print("esq ");
                result = pesquisar(title, i.esq, log);
                log.setComparacoes(log.getComparacoes()+3);
            } else {
                log.setComparacoes(log.getComparacoes()+3);
            }
        } 
...
class Show {
    ...
    */
    ...
    public static int comparelessdate ( Show a, Show b ) {
    ...
    // Ler
    public void ler ( Arvore disneyplus, String id_string ) { 
        try {
            ...
            while( s.compareTo("FIM") != 0 && !encontrou) {
                id_busca = s.split(",")[0];
                if (id_busca.compareTo(id_string) == 0) {
                    encontrou = true;
                } else {
                    s = o.nextLine();
                }
    ...
    public static void main ( String args[] ) {
        try {
            ...
            double now, time;
            FileWriter logPtr = new FileWriter("matrícula_arvoreArvore.txt");
            Log log = new Log();
            ...
            Arvore disneyplus = new Arvore();

            disneyplus.inserir(7);
            disneyplus.inserir(3);
            disneyplus.inserir(11);
            disneyplus.inserir(1);
            disneyplus.inserir(5);
            disneyplus.inserir(9);
            disneyplus.inserir(13);
            disneyplus.inserir(0);
            disneyplus.inserir(2);
            disneyplus.inserir(4);
            disneyplus.inserir(6);
            disneyplus.inserir(8);
            disneyplus.inserir(10);
            disneyplus.inserir(12);
            disneyplus.inserir(14);

            Show objeto = null; // Criar null para instanciar cada vez que chamar ler

            while( s.compareTo("FIM") != 0 ) {
                objeto = new Show();
                objeto.ler(disneyplus,s);
                s = o.nextLine();
            }

            s = o.nextLine();
            while( s.compareTo("FIM") != 0 ) {
                disneyplus.pesquisar(s,log);
                s = o.nextLine();
            }
            ...
            logPtr.write("782195\t" + log.getComparacoes() + "\t" + log.getStopwatch());
            logPtr.close();
        }
        catch ( Exception e ) {
            e.printStackTrace();
