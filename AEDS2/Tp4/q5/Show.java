...
class Hash {
    ...
    public boolean inserir(Show elemento) {
        ...
        if (elemento != null) {
            ...
            if (tabela.get(pos).get_title().compareTo("") == 0) {
                ...
                tabela.set(m1+reserva, elemento);
                reserva++;
                resp = true;
            }
        }
    ...
    public boolean pesquisar(String elemento, Log log) {
        ...
        } else if (tabela.get(pos).get_title().compareTo("") != 0) {
            log.setComparacoes(log.getComparacoes()+2);
            for (int i = 0; i < reserva; i++) {
                log.setComparacoes(log.getComparacoes()+1);
                if (tabela.get(m1+i).get_title().compareTo(elemento) == 0) {
                    resp = true;
                    pos = m1+i;
                    System.out.println(" (Posicao: " + pos + ") SIM");
                    i = reserva;
    ...
    */
    ...
    public static void main ( String args[] ) {
        try {
            ...
            double now, time;
            FileWriter logPtr = new FileWriter("matrícula_hashReserva.txt");
            Log log = new Log();
