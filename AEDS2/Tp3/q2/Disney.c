...
void inserirInicio(Show *show, Show *disneyplus[] ) {
    if ( n < 400 ) {
        for ( int y = n; y > 0; y-- ) {
            disneyplus[y] = disneyplus[y-1];
        }
        disneyplus[0] = show;
        n++;
    } else {
...
void inserirFim(Show *show, Show *disneyplus[]) {
    if ( n < 400 ) {
        disneyplus[n++] = show;
    } else {
        perror("Lista cheia!");
...
void inserir(Show* show, Show *disneyplus[], int pos) {
    if ( n < 400 && pos >= 0 && pos <= n ) {
        for ( int y = n; y > pos; y-- ) {
            disneyplus[y] = disneyplus[y-1];
        }
        disneyplus[pos] = show;
        ...
        perror("Lista cheia ou posicao errada!");
    }
}

void removerInicio(char **bin, Show *disneyplus[]) {
    if ( n > 0 ) {
        bin[binN++] = strdup(disneyplus[0]->title);
        n--;
        for ( int y = 0; y < n; y++ ) {
            disneyplus[y] = disneyplus[y+1];
        }
    } else {
        perror("Lista vazia para remocao!");
    }
}

void removerFim(char **bin, Show *disneyplus[]) {
    if ( n > 0 ) {
        bin[binN++] = strdup(disneyplus[--n]->title);
    } else {
        perror("Lista vazia para remocao!");
    }
}

void remover(char **bin, Show *disneyplus[], int pos) {
    if ( n > 0 && pos >= 0 && pos < n) {
        bin[binN++] = strdup(disneyplus[pos]->title);
        n--;
        for ( int y = pos; y < n; y++ ) {
            disneyplus[y] = disneyplus[y+1];
        }
    } else {
        perror("Lista vazia para remocao ou posicao errada!");
    }
}
 ...
 */
void read (char *s, Show *disneyplus[], char modo[], int posicao) {
    FILE *dptr;
    ...
    if (encontrado)
    {
        ...
        if ( strcmp(modo,"II") == 0 ) {
            inserirInicio(show, disneyplus);
        } else if ( strcmp(modo,"IF") == 0 ) {
            inserirFim(show, disneyplus);
        } else {
            inserir(show,disneyplus,posicao);
        }
    }
...
int main ( ) {
    ...
    while ( strcmp(s,"FIM") != 0 ) {
        read(s,disneyplus, "IF", -1);
        fgets(s, sizeof(s), stdin);
        s[strcspn(s, "\n")] = '\0';
    }

    int numbOp;
    char **operations = malloc(5*sizeof(char*));

    char **bin = malloc(200*sizeof(char*));

    scanf("%d", &numbOp);
    fgets(s, sizeof(s), stdin); //skip \n no final do numero da entrada
    fgets(s, sizeof(s), stdin);
    s[strcspn(s, "\n")] = '\0';
    for ( int y = 0; y < numbOp; y++ ) {
        operations = split(' ', s);
        if ( strcmp(operations[0],"II") == 0 ) {
            read(operations[1],disneyplus, "II", -1);
        } else if ( strcmp(operations[0],"IF") == 0 ) {
            read(operations[1],disneyplus, "IF", -1);
        } else if ( strcmp(operations[0],"I*") == 0 ) {
            read(operations[2],disneyplus, "I*", atoi(operations[1]));
        } else if ( strcmp(operations[0],"RI") == 0 ) {
            removerInicio(bin,disneyplus);
        } else if ( strcmp(operations[0],"RF") == 0 ) {
            removerFim(bin,disneyplus);
        } else if ( strcmp(operations[0],"R*") == 0 ) {
            remover(bin,disneyplus,atoi(operations[1]));
        }
        fgets(s, sizeof(s), stdin);
        s[strcspn(s, "\n")] = '\0';
    }
