...
class Lista {
    ...
    public Show removerInicio(ArrayList<String> bin) throws Exception {
        if (n == 0) {
        ...
        bin.add(resp.get_title());
        return resp;
    }

    public Show removerFim(ArrayList<String> bin) throws Exception {
        if (n == 0) {
            throw new Exception("Erro ao remover!");
        }

        bin.add(lista[--n].get_title());
        return lista[n];
    }

    public Show remover(ArrayList<String> bin, int pos) throws Exception {
        if (n == 0 || pos < 0 || pos >= n) {
            throw new Exception("Erro ao remover!");
        }
    ...
    public void mostrar() {
        for ( int y = 0; y < n; y++ ) {
            System.out.println(lista[y].get_id() + " " + lista[y].get_title());
        }
    }
}
...
class Show {
    ...
    */
    ...
    // Ler
    public void ler ( Lista disneyplus, Inserir insercao ) { 
        try {
            ...
            } else if ( insercao.modo.compareTo("II") == 0 ) {
                disneyplus.inserirInicio(this);
            } else {
                disneyplus.inserir(this, insercao.pos);
            }
    ...
    public static void main ( String args[] ) {
        try {   
            ...
            for ( int y = 0; y < number; y++ ) {
                ...
                s = o.nextLine();
                operator = s.split(" ")[0];
                if ( operator.compareTo("II") == 0 ) {
                    id = s.split(" ")[1];
                    Inserir insercao = new Inserir(operator, id, -1);
                    objeto.ler(disneyplus,insercao);
                } else if ( operator.compareTo("IF") == 0 ) {
                    id = s.split(" ")[1];
                    Inserir insercao = new Inserir(operator, id, -1);
                    objeto.ler(disneyplus,insercao);
                } else if ( operator.compareTo("I*") == 0 ) {
                    index = s.split(" ")[1];
                    id = s.split(" ")[2];
                    Inserir insercao = new Inserir(operator, id, Integer.parseInt(index));
                    objeto.ler(disneyplus,insercao);
                } else if ( operator.compareTo("RI") == 0 ) {
                    disneyplus.removerInicio(bin);
                } else if ( operator.compareTo("RF") == 0 ) {
                    disneyplus.removerFim(bin);
                } else if ( operator.compareTo("R*") == 0 ) {
                    index = s.split(" ")[1];
                    disneyplus.remover(bin,Integer.parseInt(index));
                }
            }

            for ( int y = 0; y < bin.size(); y++ ) {
                System.out.println("(R) " + bin.get(y));
            }
            for ( int y = 0; y < disneyplus.n; y++ ) {
                disneyplus.lista[y].print();;
            }
        }
        catch ( Exception e ) {
            e.printStackTrace();
