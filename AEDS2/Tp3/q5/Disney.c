...
void start () {
    Show *padrao = createDefaultShow();
    primeiro = novaCelula(padrao);
    ultimo = primeiro;
}
...
void removerInicio(char **bin) {
    if (primeiro == ultimo) {
        perror("Lista vazia para remocao!");
    } else {
        Celula* tmp = primeiro;
        primeiro = primeiro->prox;
        bin[binN++] = strdup(primeiro->elemento->title);
        tmp->prox = NULL;
        free(tmp);
        tmp = NULL;
        n--;
...
void removerFim(char **bin) {
    if (primeiro == ultimo) {
        perror("Lista vazia para remocao!");
    } else {
        Celula* i;
        for(i = primeiro; i->prox != ultimo; i = i->prox);
        bin[binN++] = strdup(ultimo->elemento->title);
        ultimo = i;
        free(ultimo->prox);
        i = ultimo->prox = NULL;
        n--;
...
void remover(char **bin, int pos) {
    if (primeiro == ultimo){
        ...
        for(j = 0; j < pos; j++, i = i->prox);
        Celula* tmp = i->prox;
        bin[binN++] = strdup(tmp->elemento->title);
        i->prox = tmp->prox;
        tmp->prox = NULL;
        free(tmp);
        i = tmp = NULL;
