...
char *removerInicio(Show *disneyplus[]) {
    if ( ultimo == primeiro ) {
        perror("Lista vazia para remocao!");
    } else {
        char *resp = strdup(disneyplus[primeiro]->title);
...
void inserirFim(Show *show, Show *disneyplus[]) {
    if (((ultimo + 1) % 6) == primeiro) {
        removerInicio(disneyplus);
    } 
    disneyplus[ultimo] = show;
    ultimo = (ultimo + 1) % 6;

    int media = 0;
    int count = 0;
    for ( int y = primeiro; y != ultimo; y = (y + 1) % 6, count++ ) {
        media += disneyplus[y]->release_year;
    }
    media /= count;
    printf("[Media] %d\n", media);
}
...
int main ( ) {
    ...
    int x = 0;
    for ( int y = primeiro; y != ultimo; y = (y + 1) % 6 ) {
        printf("[%d] ", x++);
        print(disneyplus[y]);
    }
